...
function AddToCartButton({ product, disabled }: AddToCartButtonProps) {
  ...
  return (
    <div className="flex items-center justify-center space-x-2">
      ...
      >
        <span
          className={`text-xl font-bold ${itemCount === 0 ? "text-gray-400" : "text-gray-600"} mb-0.5`}
        >
          -
        </span>
      </button>
      ...
      <button
        ...
        className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors duration-200 ${disabled ? "bg-gray-400 cursor-not-allowed" : "bg-blue-400 hover:bg-blue-600"}`}
        disabled={disabled}
      >
        <span className={`text-xl font-bold text-white mb-0.5`}>+</span>
      </button>
    </div>
  );
}
